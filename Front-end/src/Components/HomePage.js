...
class HomePage extends React.Component {
  ...
  fetchNewsData(isFromGuardian) {
    if(isFromGuardian) {
      fetch("https://jayhsieh-csci571-hw8-backend.azurewebsites.net/api/guaridan/all")
      .then(res => res.json())
      .then(
        (jsonObj) => {
      ...
      fetch("https://jayhsieh-csci571-hw8-backend.azurewebsites.net/api/nytimes/all")
      .then(res => res.json())
      .then(
        (jsonObj) => {
          this._isMounted && this.setState({
  ...
  render() {
    ...
    if (this.state._error) {
      ...
      if(this.props.isFromGuardian) {
        ...
        return (
          <div style={{width: '100%', display: 'grid', rowGap: '1rem'}}>
            {this.state._articles.map(_article => (<NewsCard key={_article._url} src = {'nytimes'} article = {_article}/>))}
          </div>
        );
      }
    }
