...
class TechnologyPage extends React.Component {
  ...
  fetchNewsData(isFromGuardian) {
    if(isFromGuardian) {
      fetch("https://jayhsieh-csci571-hw8-backend.azurewebsites.net/api/guaridan/technology")
      .then(res => res.json())
      .then(
        (jsonObj) => {
      ...
      fetch("https://jayhsieh-csci571-hw8-backend.azurewebsites.net/api/nytimes/technology")
      .then(res => res.json())
      .then(
        (jsonObj) => {
          this._isMounted && this.setState({
