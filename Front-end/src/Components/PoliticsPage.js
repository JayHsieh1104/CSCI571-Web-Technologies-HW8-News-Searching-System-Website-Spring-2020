...
class PoliticsPage extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      _error: null,
  ...
  fetchNewsData(isFromGuardian) {
    if(isFromGuardian) {
      fetch("https://jayhsieh-csci571-hw8-backend.azurewebsites.net/api/guaridan/politics")
      .then(res => res.json())
      .then(
        (jsonObj) => {
      ...
      fetch("https://jayhsieh-csci571-hw8-backend.azurewebsites.net/api/nytimes/politics")
      .then(res => res.json())
      .then(
        (jsonObj) => {
          this._isMounted && this.setState({
